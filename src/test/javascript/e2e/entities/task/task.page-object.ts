...
export class TaskUpdatePage {
  ...
  async typeSelectLastOption(): Promise<void> {
    ...
    await this.typeSelect.all(by.tagName('.ui-dropdown-item')).last().click();
  }
  ...
  isDoneInputSelected(): Promise<boolean> {
    return this.getDoneInput().element(by.css('input[type="checkbox"]')).isSelected() as Promise<boolean>;
  }
  ...
  async getAttachmentInput(): Promise<string> {
    return await this.attachmentInput.all(by.css('.ui-fileupload-row > div')).get(1).getText();
  }
  ...
  async getPictureInput(): Promise<string> {
    return await this.pictureInput.all(by.css('.ui-fileupload-row > div')).get(1).getText();
  }

  async userSelectLastOption(): Promise<void> {
    await this.userSelect.click();
    await this.userSelect.all(by.tagName('.ui-dropdown-item')).last().click();
  }
